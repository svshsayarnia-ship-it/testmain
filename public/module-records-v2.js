...
function savePartRequest(){var v={};document.querySelectorAll("#modal [data-part-field]").forEach(function(el){v[el.getAttribute("data-part-field")]=el.type==="number"?+el.value:el.value});var r=K.requestPart(v),m=document.getElementById("modal");if(m)m.innerHTML="";var b=document.getElementById("toasts");if(b){var msg=r.purchaseRequest?"درخواست خرید فقط به دلیل کسری یا حد سفارش ایجاد شد.":r.request.existingResponseId?"از درخواست یا سفارش موجود استفاده شد؛ خرید تکراری ایجاد نشد.":"قطعه از انبار خارج شد؛ خریدی ایجاد نشد.";var t=document.createElement("div");t.className="toast";t.innerHTML="<b>درخواست قطعه ثبت شد</b><small>"+msg+"</small>";b.appendChild(t);setTimeout(function(){t.remove()},4000)}}
