...
function runAcceptanceSuite(){
 ...
 try{
  ...
  test("Inventory data change auto-creates stockout event and CEO decision",function(){setSession("USR-CEO");var r=upsertBusinessRecord("inventoryStock",{id:"STK-TEST",module:"inventory",itemId:"ITEM-6205",name:"رولبرینگ SKF 6205",available:0,safety:8,coverageDays:0,etaDays:5,critical:true,emergencyPurchaseAmount:780000000});return r._derivedEvents.length>0&&db.events.some(function(e){return e.type==="Inventory.Stockout"&&e.entityId==="ITEM-6205"})&&db.decisions.some(function(d){return d.key==="inventory:ITEM-6205:emergency-buy"})});
  test("Production data change auto-creates target miss event",function(){var r=upsertBusinessRecord("productionShift",{id:"SHIFT-TEST",module:"production",entityId:"SHIFT-TEST",plan:100,actual:80,downtimeMin:5});return r._derivedEvents.length>0&&db.events.some(function(e){return e.type==="Production.TargetMissed"&&e.entityId==="SHIFT-TEST"})});
  test("Normal inventory resolves derived stock exception",function(){upsertBusinessRecord("inventoryStock",{id:"STK-TEST",module:"inventory",itemId:"ITEM-6205",name:"رولبرینگ SKF 6205",available:20,safety:8,coverageDays:20,etaDays:5,critical:true});return !db.events.some(function(e){return e.status==="Active"&&e.type==="Inventory.Stockout"&&e.entityId==="ITEM-6205"})});
  test("Maintenance WO auto-correlates to production impact",function(){var r=upsertBusinessRecord("maintenanceWO",{id:"WO-AUTO",module:"maintenance",assetId:"AST-ELV2",status:"Emergency",downtimeMin:45,productionImpact:true,critical:true,partIds:["ITEM-6205"]});return r._derivedEvents.length>0&&db.events.some(function(e){return e.type==="Maintenance.AssetFailed"&&e.entityId==="AST-ELV2"})&&db.events.some(function(e){return e.type==="Production.DowntimeExceeded"&&e.rootEventId})});
  test("Quality hold creates case and shipment impact",function(){var r=upsertBusinessRecord("qualityBatch",{id:"B-AUTO",module:"quality",batchId:"B-2405",status:"Hold",shipmentBlocked:true,shipmentIds:["ORD-91"]});return r._derivedEvents.length>0&&db.cases.some(function(x){return x.key==="quality:B-2405"})&&db.events.some(function(e){return e.type==="Sales.ShipmentBlocked"&&e.entityId==="ORD-91"})});
  test("Finance negative cash gap creates management case",function(){var r=upsertBusinessRecord("financeCash",{id:"CASH-AUTO",module:"finance",department:"Finance",sensitivity:"confidential",netGap:-2000000000,status:"At Risk"});return r._derivedEvents.length>0&&db.events.some(function(e){return e.type==="Finance.CashRisk"&&e.entityId==="CASH-AUTO"})&&db.cases.some(function(x){return x.module==="finance"})});
  test("HSE high incident enters emergency flow",function(){var r=upsertBusinessRecord("hseIncident",{id:"HSE-AUTO",module:"hse",department:"HSE",sensitivity:"restricted",title:"حادثه مهم تست",severity:"Critical",status:"Reported"});return r._derivedEvents.length>0&&db.notifications.some(function(n){return n.priority==="P0"&&n.recipient==="مدیرعامل"})});
  test("Critical project milestone delay creates recovery case",function(){var r=upsertBusinessRecord("projectMilestone",{id:"MS-AUTO",module:"projects",department:"Projects",project:"پروژه تست",criticalPath:true,delayDays:8,status:"Delayed"});return r._derivedEvents.length>0&&db.cases.some(function(x){return x.module==="projects"&&x.status!=="Closed"})});
  test("Unauthorized security access creates live exception",function(){var r=upsertBusinessRecord("securityAccess",{id:"SEC-AUTO",module:"security",department:"Security",sensitivity:"restricted",person:"X",area:"سیلو",authorized:false,status:"Denied"});return r._derivedEvents.length>0&&db.events.some(function(e){return e.type==="Security.UnauthorizedAccess"&&e.entityId==="SEC-AUTO"})});
  test("Tax near deadline creates owner action",function(){var r=upsertBusinessRecord("taxObligation",{id:"TAX-AUTO",module:"tax",department:"Finance",sensitivity:"confidential",type:"VAT",daysToDue:2,status:"Open"});return r._derivedEvents.length>0&&db.actions.some(function(a){return a.module==="tax"&&!/Completed|Verified/.test(a.status)})});
